...
class O19Injections:
    # Exceptions to avoid the blocking request. Only for trusted mods which create requests to localhost or listen on localhost.
    # hash is the fast method to verify the mod and sha256 will only be calculated if hash matches.
    # Trivial patterns which match 'hash' will not match 'sha'
    # _call = f"{filename}:{function}" // text = ...
    # hash(_call) // hashlib.sha256(_call.encode('UTF-8')).hexdigest()
    bypass_text = ['USB dongle', ]
    bypass_hash = [7774788719015408404, ]
    bypass_sha = ['671e2ba9be23d41eb870709af763fb915b10fceca895ba3181ba1ab3ba3b5d4a', ]

    @staticmethod
    def log_data(_method: str, *args, **kwargs):

        _stack: List[str] = []
        log_message = f"'{_method}({args}; {kwargs})' called"
        log.warn(log_message)
        try:
            log_message = f"'{log_message} from:\n"
            for thread in threading.enumerate():
                ...
                for thread_detail in thread_details:
                    ...
                    _stack.append(f"{filename}:{function}")
        ...
        for _call in _stack:
            _hash = hash(_call)
            log.debug(f"{_call} = {_hash}")
            if _hash in O19Injections.bypass_hash:
                _hash = hashlib.sha256(_call.encode('UTF-8')).hexdigest()
                log.debug(f"{_call} = {_hash}")
                if _hash in O19Injections.bypass_sha:
                    log.warn(f"Bypassing '{O19Injections.bypass_text[O19Injections.bypass_sha.index(_hash)]}'")
                    return
...
try:
    ...
    @S4CLCommonInjectionUtils.inject_safely_into(S4CLModInfo.get_identity(), HTTPConnection, HTTPConnection.__init__.__name__, handle_exceptions=False)
    def o19_injected_http_client_httpconnection(original, self, *args, **kwargs):
        O19Injections.log_data('http.client.HTTPConnection', *args, **kwargs)
        return original(self, *args, **kwargs)


    log.debug("Injected into http.client.HTTPConnection()")
except Exception as e1:
    try:
        import http.client

        o19_org_http_client_httpconnection = http.client.HTTPConnection


        def o19_override_http_client_httpconnection(*args, **kwargs):
            O19Injections.log_data('http.client.HTTPConnection', *args, **kwargs)
            return o19_org_http_client_httpconnection(*args, **kwargs)


        http.client.HTTPConnection = o19_override_http_client_httpconnection
        log.debug("Replaced http.client.HTTPConnection()")
    except Exception as e2:
        log.warn(f"'http.client.HTTPConnection()': Could not inject '{e1}' or replace '{e2}' ")
